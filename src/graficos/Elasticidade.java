...
//                timeseries3.addOrUpdate(p, Double.parseDouble(String.valueOf(log.getCpuIdle())));
                ...
                double z = Double.parseDouble(String.valueOf(log.getMemoriaFree()));
                if (menor > x) {menor = x;}
                if (menor > y) {menor = y;}
                if (menor > z) {menor = z;}
                if (maior < x) {maior = x;}
...
//        timeseriescollection.addSeries(timeseries3);
    ...
    public void criaGrafico(JPanel jPanel1) throws IOException {
        JFreeChart jfreechart = ChartFactory.createTimeSeriesChart("Elasticidade", "Data", "", createDataset(), true, true, false);
