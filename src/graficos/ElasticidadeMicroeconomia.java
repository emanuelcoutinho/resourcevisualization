...
public class ElasticidadeMicroeconomia {
    ...
    public ElasticidadeMicroeconomia() {
        this.mv = 6;
    }

    public ElasticidadeMicroeconomia(int mv) {
        this.mv = mv;
    }
    ...
    // Tempo	SQN	TRA	TRD	ERA	ERD	ED
        ...
        maior = Double.parseDouble(String.valueOf(inicial.getMemoriaFree()));
        menor = Double.parseDouble(String.valueOf(inicial.getMemoriaFree()));
...
//                Date d = Datas.stringToDataShellScript(log.getData());
                ...
                double x = Double.parseDouble(String.valueOf(log.getMemoriaFree()));
                if (menor > x) {menor = x;}
                if (maior < x) {maior = x;}
...
//        timeseriescollection.addSeries(timeseries5);
    ...
    public void criaGrafico(JPanel jPanel1) throws IOException {
        JFreeChart jfreechart = ChartFactory.createTimeSeriesChart("Elasticidade Microeconomia", "Data", "", createDataset(), true, true, false);
...
//        xyitemrenderer.setSeriesPaint(4, Color.ORANGE);
...
//        numberAxis.setRange(0, 3);
        
        double tick = Math.abs(menor) + Math.abs(maior);
        if (tick<=10) tick = 1;
        if ((tick>15) && (tick<=20)) tick = 2;
        if ((tick>20) && (tick<=25)) tick = 3;
        if ((tick>25) && (tick<=30)) tick = 4;
        if (tick>30) tick = 5;
